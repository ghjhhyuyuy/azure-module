...
storage_os_disk {
    name          = "myosdisk1"
    vhd_uri       = "${azurerm_storage_account.frontend.primary_blob_endpoint}${azurerm_storage_container.example.name}/myosdisk1.vhd"
    caching       = "ReadWrite"
...
resource "azurerm_virtual_machine_extension" "frontend" {
  ...
  settings = <<SETTINGS
    {
     "fileUris":["https://testzuowen.blob.core.windows.net/nginx/install_nginx.sh"], "commandToExecute": "sh install_frontend.sh" 
    }
SETTINGS
